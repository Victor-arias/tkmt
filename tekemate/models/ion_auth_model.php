...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 **/
	public function activate($Id_usuario, $code = false)
	{
	    if ($code !== false)
	    {
	    ...
	    else
	    {
			...
			$this->db->where($this->ion_auth->_extra_where);
			$this->db->update($this->tables['users'], $data, array('Id_usuario' => $Id_usuario));
	    }
	 ...
	 * @author Mathew
	 **/
	public function deactivate($Id_usuario = 0)
	{
	    if (empty($Id_usuario))
	    {
		return FALSE;
	...
	public function profile($identity = '', $is_code = false)
	{
	    ...
	    $this->db->join($this->tables['meta'], $this->tables['users'].'.Id_usuario = '.$this->tables['meta'].'.'.$this->meta_join, 'left');
	    $this->db->join($this->tables['groups'], $this->tables['users'].'.Id_grupo = '.$this->tables['groups'].'.Id_grupo', 'left');
	...
	public function register($username, $password, $email, $additional_data = false, $group_name = false)
	{
	    ...
	    if(isset($additional_data['Id_grupo']))
	    {
		$Id_grupo = $additional_data['Id_grupo'];
		unset($additional_data['Id_grupo']);
	    }
		...
		$Id_grupo = $this->db->select('Id_grupo')
	    ...
	    // Meta table.
	    $Id_meta_usuario = $this->db->insert_id();

	    $data = array($this->meta_join => $Id_meta_usuario);
	    ...
	    return $this->db->affected_rows() > 0 ? $Id_meta_usuario : false;
	}
	...
	public function login($identity, $password, $remember=FALSE)
	{
	    ...
	    $query = $this->db->select($this->identity_column.', Id_usuario, password, Id_grupo, username')
			      ->where($this->identity_column, $identity)
			      ->where('active', 1)
			      ->where($this->ion_auth->_extra_where)
			      ->limit(1)
		...
		{
		    $this->update_last_login($result->Id_usuario);

		    $group_row = $this->db->select('name')->where('Id_grupo', $result->Id_grupo)->get($this->tables['groups'])->row();

		    $session_data = array(
					$this->identity_column => $result->{$this->identity_column},
					//'id'                   => $result->id, //kept for backwards compatibility
					'Id_usuario'              => $result->Id_usuario, //everyone likes to overwrite id so we'll use Id_usuario
		    ...
		    {
			$this->remember_user($result->Id_usuario);
		    }
	 ...
	 **/
	public function get_user($Id_usuario = false)
	{
	    ...
	    $this->db->where($this->tables['users'].'.Id_usuario', $Id_usuario);
	    $this->db->limit(1);
	 ...
	 * @author Ben Edmunds
	 **/
	public function get_users_group($Id_grupo=false)
	{
	    //if no id was passed use the current users id
	    $id || $id = $this->session->userdata('Id_usuario');

	    $user = $this->db->select('Id_grupo')
			     ->where('Id_grupo', $Id_grupo)
			     ->get($this->tables['users'])
			     ->row();

	    return $this->db->select('name, description')
			    ->where('Id_grupo', $user->Id_grupo)
			    ->get($this->tables['groups'])
			    ->row();
	}
	 ...
	 * @author Ben Edmunds
	 **/
	public function get_group($Id_grupo)
  	{
	...
	{
	    $user = $this->get_user($Id_usuario)->row();
	    ...
	    if (!empty($this->columns))
	    {
			...
			{
				// 'Id_usuario' = $id
				$this->db->where($this->meta_join, $Id_usuario);
				$this->db->set($meta_fields);
				$this->db->update($this->tables['meta']);
			}
	    }

	    if (array_key_exists('username', $data) || array_key_exists('password', $data) || array_key_exists('email', $data) || array_key_exists('Id_grupo', $data))
			{
	...
	public function delete_user($Id_usuario)
	{
	    ...
	    $this->db->delete($this->tables['meta'], array($this->meta_join => $Id_usuario));
	    $this->db->delete($this->tables['users'], array('Id_usuario' => $Id_usuario));
	 ...
	 * @author Ben Edmunds
	 **/
	public function update_last_login($Id_usuario)
	{
	    $this->load->helper('date');
	    ...
	    $this->db->update($this->tables['users'], array('last_login' => now()), array('Id_usuario' => $Id_usuario));
	...
	public function login_remembered_user()
	{
	    ...
	    $query = $this->db->select($this->identity_column.', Id_usuario, Id_grupo')
		...
		$group_row = $this->db->select('name')->where('Id_grupo', $user->Id_grupo)->get($this->tables['groups'])->row();

		$session_data = array(
				    $this->identity_column => $user->{$this->identity_column},
				    //'id'                   => $user->id, //kept for backwards compatibility
				    'Id_usuario'              => $user->Id_usuario, //everyone likes to overwrite id so we'll use Id_usuario
	 ...
	 * @author Ben Edmunds
	 **/
	private function remember_user($Id_usuario)
	{
	    if (!$Id_usuario)
	    {
			return FALSE;
	    ...
	    $this->db->update($this->tables['users'], array('remember_code' => $salt), array('Id_usuario' => $Id_usuario));
...
/* Location: ./tekemate/models/ion_auth_model.php */